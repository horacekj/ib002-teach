...
\begin{enumerate}
	...
	\begin{procedure}[H]
		...
		\Input{vrchol $u$, množina navštívených vrcholů $\mathit{Visited}$}
		\Output{množina navštívených vrcholů $\mathit{Visited}$}
		\BlankLine
		$\mathit{Visited} \leftarrow \mathit{Visited} \cup \{u\}$\;
		\For{$v \in \mathit{u.successors}$} {
			\If{$v \not\in \mathit{Visited}$} {
				$\mathit{Visited} \leftarrow $\ffVisitedDFS{$v$, $\mathit{Visited}$} \;
			}
		}
